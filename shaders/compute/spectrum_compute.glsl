...
layout(push_constant) restrict readonly uniform PushConstants {
	...
	float detail;
	float spread;
	float k_min;   // cascade band, lower bound (rad/m)
	float k_max;   // cascade band, upper bound (rad/m)
	uint cascade_index;
	// Godot 4.7 validates push-constant size exactly; the GDScript side rounds up
	// to 16 bytes, so the struct is padded to match rather than shrinking the pad.
	uint pad0;
};
...
vec2 get_spectrum_amplitude(in ivec2 id, in ivec2 map_size) {
	...
	// Cascade band limit. Without it every cascade carries the whole spectrum and
	// the same wave is summed N times -- the sea comes out N times too steep and
	// the cascades beat against each other. Each cascade owns one octave band and
	// the edges are feathered so no energy falls down the cracks between them.
	if (k < k_min*0.5 || k > k_max*2.0) return vec2(0.0);
	float band = smoothstep(k_min*0.5, k_min, k) * (1.0 - smoothstep(k_max, k_max*2.0, k));
	...
	return gaussian(hash(uvec2(id + seed))) * sqrt(2.0 * s * d * w_norm * band);
}
