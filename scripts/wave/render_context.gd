...
## A 2D view onto one layer at one mip level of an array texture, so a compute
## pass can write it. 4.7 offers no 2D-array slice, hence per-layer.
func create_mip_slice(texture : RID, layer : int, mipmap : int) -> Descriptor:
	var view := RDTextureView.new()
	var rid := device.texture_create_shared_from_slice(view, texture, layer, mipmap, 1,
			RenderingDevice.TEXTURE_SLICE_2D)
	return Descriptor.new(deletion_queue.push(rid), RenderingDevice.UNIFORM_TYPE_IMAGE)

## Creates a descriptor set. The ordering of the provided descriptors matches the binding ordering
## within the shader.
func create_descriptor_set(descriptors : Array, shader : RID, descriptor_set_index :=0) -> RID:
