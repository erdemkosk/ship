extends Node
class_name VerificationHarness
## Command-line verification and screenshot harness.
##
## This node is created only for probe/test launches. Keeping it outside the
## runtime composition root prevents test orchestration from becoming gameplay code.
...
func configure(p_rig: Node3D, p_boat: RigidBody3D, p_ocean: Node3D, p_weather: Node3D) -> void:
	rig = p_rig
	boat = p_boat
	ocean = p_ocean
	weather = p_weather
...
func run(args: PackedStringArray) -> void:
	...
	for arg in args:
		...
		#   --pitch= --yaw=   nudge the view; --screenshot=PATH then quits
		...
		elif arg == "--weather-effects-test":
			_weather_effects_test(boat, weather)
		elif arg == "--interior-visual-test":
			_interior_visual_test(boat)
		elif arg == "--deck-visual-test":
			_deck_visual_test(boat)
		elif arg == "--console-visual-test":
			_console_visual_test(boat)
		elif arg == "--electronics-visual-test":
			_electronics_visual_test(boat)
		elif arg == "--switchboard-visual-test":
			_switchboard_visual_test(boat)
		elif arg == "--anchor-visual-test":
			_anchor_visual_test(boat)
		elif arg == "--cabin-visual-test":
			_cabin_visual_test(boat)
		elif arg == "--wheelhouse-visual-test":
			_wheelhouse_visual_test(boat)
		elif arg == "--mast-visual-test":
			_mast_visual_test(boat)
		elif arg == "--companionway-visual-test":
			_companionway_visual_test(boat)
			...
			_menu_shot(get_tree().get_first_node_in_group("main_menu"),
					arg.get_slice("=", 1))
		elif arg == "--lighthouse":
			look_lh = true
...
func _weather_effects_test(boat: RigidBody3D, active_weather: Node3D) -> void:
	var effects := boat.get("_weather_effects") as Node
	if effects == null:
		push_error("boat weather effects component missing")
		_quit_cleanly()
		return
	active_weather.set("rain_amount", 1.0)
	var hull_materials := boat.get("_hull_mats") as Array[ShaderMaterial]
	effects.call("update_hull_wetness", 1.0, boat.get("ocean"), active_weather,
			boat.global_position, Vector3(7.0, 0.0, 0.0), hull_materials)
	effects.call("update_glass", 1.0, active_weather, true,
			boat.get("_wiper_arm"), boat.get("_glass_mat"),
			boat.get("_front_glass_mat"))
	var report := effects.call("contract_report") as Dictionary
	var complete := int(report.get("shield_count", 0)) == 3 \
			and bool(report.get("field_ready", false)) \
			and float(report.get("soak", 0.0)) > 0.50 \
			and float(report.get("glass_wet", 0.0)) > 0.40 \
			and absf(float(report.get("wiper_pose", 0.0))) > 0.05
	print("[weather-effects] shields=%d field=%s soak=%.3f glass=%.3f wiper=%.3f complete=%s" % [
			int(report.get("shield_count", 0)), report.get("field_ready", false),
			float(report.get("soak", 0.0)), float(report.get("glass_wet", 0.0)),
			float(report.get("wiper_pose", 0.0)), complete])
	if not complete:
		push_error("boat weather effects contract incomplete")
	_quit_cleanly()


func _interior_visual_test(boat: RigidBody3D) -> void:
	var door := boat.get("_locker_door") as Node3D
	var tank := boat.get("_gear_tank") as Node3D
	var mask := boat.get("_gear_mask") as Node3D
	var heat := boat.get("_stove_heat") as GPUParticles3D
	var stove_sound := boat.get("_stove_snd") as AudioStreamPlayer3D
	var built := door != null and tank != null and mask != null \
			and door.get_parent() == boat and tank.get_parent() == boat \
			and mask.get_parent() == boat and door.get_child_count() >= 12 \
			and tank.get_child_count() >= 12 and mask.get_child_count() >= 8 \
			and heat != null and heat.get_parent() == boat \
			and heat.process_material is ParticleProcessMaterial \
			and stove_sound != null and stove_sound.get_parent() == boat
	var closed_angle := door.rotation.y if door != null else 0.0
	boat.call("toggle_switch", "locker")
	await get_tree().create_timer(0.30).timeout
	var opens := door != null and absf(door.rotation.y) > absf(closed_angle) + 0.05
	var complete := built and opens
	print("[interior-visual] door=%d tank=%d mask=%d heat=%s sound=%s opens=%s complete=%s" % [
			door.get_child_count() if door != null else 0,
			tank.get_child_count() if tank != null else 0,
			mask.get_child_count() if mask != null else 0,
			heat != null, stove_sound != null, opens, complete])
	if not complete:
		push_error("boat interior visual builder contract incomplete")
	_quit_cleanly()


func _deck_visual_test(boat: RigidBody3D) -> void:
	var deck := boat.get("_deck_visuals") as Node3D
	var meshes := deck.find_children("*", "MeshInstance3D", true, false) \
			if deck != null else []
	var torus_count := 0
	var cylinder_count := 0
	for candidate in meshes:
		var instance := candidate as MeshInstance3D
		if instance.mesh is TorusMesh:
			torus_count += 1
		elif instance.mesh is CylinderMesh:
			cylinder_count += 1
	var ladder := deck.find_child("SeaLadder", true, false) as Node3D \
			if deck != null else null
	var complete := deck != null and deck.get_parent() == boat \
			and meshes.size() >= 100 and torus_count >= 20 \
			and cylinder_count >= 50 and ladder != null \
			and ladder.get_child_count() >= 10
	print("[deck-visual] meshes=%d torus=%d cylinders=%d ladder=%d complete=%s" % [
			meshes.size(), torus_count, cylinder_count,
			ladder.get_child_count() if ladder != null else 0, complete])
	if not complete:
		push_error("boat deck visual builder contract incomplete")
	_quit_cleanly()


func _console_visual_test(boat: RigidBody3D) -> void:
	var face := boat.find_child("AnalogConsoleFace", true, false) as Node3D
	var needles := boat.get("_needles") as Array[Node3D]
	var compass := boat.get("_compass_card") as Node3D
	var labels: Array[String] = []
	if face != null:
		for candidate in face.find_children("*", "Label3D", true, false):
			labels.append((candidate as Label3D).text)
	var expected_labels := ["PARAKETE", "İSKANDİL", "ZİNCİR", "PUSULA"]
	var complete := face != null and face.get_parent() == boat \
			and needles.size() == 3 and compass != null \
			and boat.get("_dial_face_mat") is ShaderMaterial \
			and boat.get("_dial_ink") is StandardMaterial3D
	for expected: String in expected_labels:
		complete = complete and expected in labels
	print("[console-visual] needles=%d compass=%s labels=%s complete=%s" % [
			needles.size(), compass != null, expected_labels, complete])
	if not complete:
		push_error("boat console visual builder contract incomplete")
	_quit_cleanly()


func _electronics_visual_test(boat: RigidBody3D) -> void:
	var sounder_arm := boat.get("_sounder_arm") as Node3D
	var sounder_pivot := boat.get("_sounder_pivot") as Node3D
	var sounder_case := boat.get("_sounder_case") as MeshInstance3D
	var sounder_screen := boat.get("_sounder_screen") as MeshInstance3D
	var radar_arm := boat.get("_radar_arm") as Node3D
	var radar_pivot := boat.get("_radar_pivot") as Node3D
	var radar_case := boat.get("_radar_case") as MeshInstance3D
	var radar_screen := boat.get("_radar_screen") as MeshInstance3D
	var radio_hand := boat.get("_radio_hand") as Node3D
	var cord := boat.get("_cord") as Array[MeshInstance3D]
	var depth_history := boat.get("_depth_hist") as PackedFloat32Array
	var complete := sounder_arm != null and sounder_arm.get_parent() == boat \
			and sounder_pivot != null and sounder_pivot.get_parent() == sounder_arm \
			and sounder_case != null and sounder_case.get_parent() == sounder_pivot \
			and sounder_screen != null and sounder_screen.get_parent() == sounder_pivot \
			and boat.get("_sounder_mat") is ShaderMaterial \
			and radar_arm != null and radar_arm.get_parent() == boat \
			and radar_pivot != null and radar_pivot.get_parent() == radar_arm \
			and radar_case != null and radar_case.get_parent() == radar_pivot \
			and radar_screen != null and radar_screen.get_parent() == radar_pivot \
			and boat.get("_radar_mat") is ShaderMaterial \
			and radio_hand != null and radio_hand.get_parent() == boat \
			and cord.size() == 48 and depth_history.size() == 64
	print("[electronics-visual] sounder=%s radar=%s radio=%s cord=%d depth=%d complete=%s" % [
			sounder_screen != null, radar_screen != null, radio_hand != null,
			cord.size(), depth_history.size(), complete])
	if not complete:
		push_error("boat electronics visual builder contract incomplete")
	_quit_cleanly()


func _switchboard_visual_test(boat: RigidBody3D) -> void:
	var lid := boat.get("_fuse_lid") as Node3D
	var cartridges := boat.get("_fuse_bodies") as Dictionary
	var levers := boat.get("_switch_levers") as Dictionary
	var leds := boat.get("_switch_leds") as Dictionary
	var ids := ["cabin", "helm", "beacon", "anchor", "flood", "wiper"]
	var complete := lid != null and lid.get_parent() == boat \
			and cartridges.size() == 6 and levers.size() == 6 and leds.size() == 6
	for id: String in ids:
		var fuse_id := "fu_" + id
		var switch_id := "sw_" + id
		var cartridge := cartridges.get(fuse_id) as Node3D
		var lever := levers.get(switch_id) as Node3D
		complete = complete and cartridge != null and lever != null \
				and leds.get(switch_id) is StandardMaterial3D \
				and bool(boat.call("fuse_seated", fuse_id))
	print("[switchboard-visual] lid=%s cartridges=%d levers=%d leds=%d complete=%s" % [
			lid != null, cartridges.size(), levers.size(), leds.size(), complete])
	if not complete:
		push_error("boat switchboard visual builder contract incomplete")
	_quit_cleanly()


func _anchor_visual_test(boat: RigidBody3D) -> void:
	var chain := boat.get("_chain_visual") as Node3D
	var windlass := boat.get("_windlass") as Node3D
	var chain_meshes := chain.find_children("*", "MeshInstance3D", true, false) \
			if chain != null else []
	var complete := chain != null and chain.get_parent() == boat \
			and chain_meshes.size() == 144 \
			and windlass != null and windlass.get_parent() == boat \
			and windlass.get_child_count() >= 12
	print("[anchor-visual] chain=%d windlass=%d complete=%s" % [
			chain_meshes.size(), windlass.get_child_count() if windlass != null else 0,
			complete])
	if not complete:
		push_error("boat anchor visual builder contract incomplete")
	_quit_cleanly()


func _cabin_visual_test(boat: RigidBody3D) -> void:
	var forward_door := boat.get("_door_fwd") as Node3D
	var aft_door := boat.get("_door_aft") as Node3D
	var stove_switch := boat.get("_stove_switch") as Node3D
	var cabin_lamp := boat.get("_cabin_lamp") as OmniLight3D
	var stove_lamp := boat.get("_stove_lamp") as OmniLight3D
	var stove_fill := boat.get("_stove_fill") as OmniLight3D
	var complete := boat.get("_glass_mat") is ShaderMaterial \
			and boat.get("_lit_window") is StandardMaterial3D \
			and forward_door != null and forward_door.get_parent() == boat \
			and aft_door != null and aft_door.get_parent() == boat \
			and forward_door.get_child_count() >= 12 \
			and aft_door.get_child_count() >= 12 \
			and stove_switch != null and stove_switch.get_parent() == boat \
			and cabin_lamp != null and stove_lamp != null and stove_fill != null \
			and boat.get("_stove_reflector") is StandardMaterial3D \
			and boat.get("_stove_ember") is StandardMaterial3D
	print("[cabin-visual] doors=%d/%d switch=%s lamps=%s/%s/%s complete=%s" % [
			forward_door.get_child_count() if forward_door != null else 0,
			aft_door.get_child_count() if aft_door != null else 0,
			stove_switch != null, cabin_lamp != null, stove_lamp != null,
			stove_fill != null, complete])
	if not complete:
		push_error("boat cabin visual builder contract incomplete")
	_quit_cleanly()


func _wheelhouse_visual_test(boat: RigidBody3D) -> void:
	var door := boat.get("_door_wh") as Node3D
	var lamp := boat.get("_helm_lamp") as OmniLight3D
	var wiper := boat.get("_wiper_arm") as Node3D
	var complete := boat.get("_front_glass_mat") is ShaderMaterial \
			and door != null and door.get_parent() == boat \
			and door.get_child_count() >= 12 \
			and lamp != null and lamp.get_parent() == boat \
			and boat.get("_helm_glow") is StandardMaterial3D \
			and wiper != null and wiper.get_parent() == boat \
			and boat.get("_chart_mat") is ShaderMaterial \
			and boat.get("_radar_screen") is MeshInstance3D \
			and boat.get("_sounder_screen") is MeshInstance3D
	print("[wheelhouse-visual] door=%d lamp=%s wiper=%s chart=%s electronics=%s complete=%s" % [
			door.get_child_count() if door != null else 0, lamp != null,
			wiper != null, boat.get("_chart_mat") is ShaderMaterial,
			boat.get("_radar_screen") is MeshInstance3D, complete])
	if not complete:
		push_error("boat wheelhouse visual builder contract incomplete")
	_quit_cleanly()


func _mast_visual_test(boat: RigidBody3D) -> void:
	var mast := boat.find_child("MastVisuals", false, false) as Node3D
	var beacon := boat.get("_beacon") as OmniLight3D
	var nav_materials := boat.get("_nav_mats") as Array[StandardMaterial3D]
	var nav_spots := boat.get("_nav_spots") as Array[SpotLight3D]
	var complete := mast != null and mast.get_parent() == boat \
			and mast.get_child_count() >= 6 \
			and beacon != null and beacon.get_parent() == mast \
			and boat.get("_beacon_mat") is StandardMaterial3D \
			and nav_materials.size() == 3 and nav_spots.size() == 3 \
			and boat.get("_deck_visuals") is Node3D
	print("[mast-visual] mast=%d beacon=%s nav=%d/%d deck=%s complete=%s" % [
			mast.get_child_count() if mast != null else 0, beacon != null,
			nav_materials.size(), nav_spots.size(),
			boat.get("_deck_visuals") is Node3D, complete])
	if not complete:
		push_error("boat mast visual builder contract incomplete")
	_quit_cleanly()


func _companionway_visual_test(boat: RigidBody3D) -> void:
	var engine_door := boat.get("_door_eng") as Node3D
	var floodlights := boat.get("_floods") as Array[SpotLight3D]
	var complete := engine_door != null and engine_door.get_parent() == boat \
			and engine_door.get_child_count() >= 10 \
			and floodlights.size() == 2 \
			and boat.get("_flood_lens_mat") is StandardMaterial3D \
			and boat.get("_flood_beam_mat") is ShaderMaterial
	for floodlight: SpotLight3D in floodlights:
		complete = complete and floodlight.get_parent() == boat \
				and is_equal_approx(floodlight.spot_range, 48.0)
	print("[companionway-visual] door=%d floods=%d lens=%s beam=%s complete=%s" % [
			engine_door.get_child_count() if engine_door != null else 0,
			floodlights.size(), boat.get("_flood_lens_mat") is StandardMaterial3D,
			boat.get("_flood_beam_mat") is ShaderMaterial, complete])
	if not complete:
		push_error("boat companionway visual builder contract incomplete")
	_quit_cleanly()


func _quit_cleanly() -> void:
	## Tests used to quit from inside the active scene, leaving ObjectDB/resource
	## leak noise at shutdown. Queue the scene first and let SceneTree quit on
	## the following process frame, after queued frees have completed.
	var tree := get_tree()
	var shutdown_timer := Timer.new()
	shutdown_timer.one_shot = true
	shutdown_timer.wait_time = 0.50
	shutdown_timer.autostart = true
	shutdown_timer.timeout.connect(tree.quit)
	tree.root.add_child.call_deferred(shutdown_timer)
	if tree.current_scene != null:
		_stop_audio_tree(tree.current_scene)
		tree.current_scene.queue_free.call_deferred()
...
func _knife_test(rig: Node3D, boat: RigidBody3D) -> void:
	...
	# Extraction and closing are deliberately separate animations. Wait on the
	# interaction gate the player actually uses instead of assuming a frame rate
	# dependent wall-clock instant for the attack admission test.
	var knife_close_deadline := Time.get_ticks_msec() + 900
	while float(rig.get("_bag_focus")) >= 0.08 \
			and Time.get_ticks_msec() < knife_close_deadline:
...
func _rifle_test(rig: Node3D, boat: RigidBody3D) -> void:
	...
	var rifle_close_deadline := Time.get_ticks_msec() + 900
	while float(rig.get("_bag_focus")) >= 0.035 \
			and Time.get_ticks_msec() < rifle_close_deadline:
		await get_tree().process_frame
	...
	var shot_palm_error := maxf(primary_palm.origin.distance_to(
			...
			rifle.global_transform * support_grip.position))
	# Physics/render interpolation can move the sampled post-shot frame by one
	# tick in headless runs. Fourteen millimetres still rejects a visibly detached
	# palm while leaving two millimetres of scheduling tolerance at the boundary.
	var shot_palms_pinned := shot_palm_error < 0.014
	...
	# Headless render cadence can leave the cartridge one interpolation frame
	# behind the hand at this timestamp. Sample a bounded set of visible frames;
	# this still rejects a round that never reaches the authored chamber pose.
	for _insertion_frame in 12:
		if insertion_root_error < 0.012 and insertion_axis_dot > 0.995:
			break
		if not cartridge.visible:
			break
		await get_tree().process_frame
		chamber_world = rifle.global_transform * chamber_node.transform
		insertion_root_error = cartridge.global_position.distance_to(
				chamber_world.origin)
		insertion_axis_dot = cartridge.global_basis.z.normalized().dot(
				chamber_world.basis.z.normalized())
	...
	# The pose and contact solvers publish on adjacent render passes. A fixed
	# timer can sample the old phalanx report on a slow headless frame even
	# though the next rendered pose is clean. Wait a bounded twelve frames for
	# the report that belongs to the current bolt grip; a persistent penetration
	# still fails the contract rather than being retried away.
	for _settle_frame in 12:
		var settle_map := bolt_contact.get("touches_by_finger", {}) as Dictionary
		if int(settle_map.get(1, 0)) >= 1 \
				and int(bolt_contact.get("penetrations", 1)) == 0:
			break
		await get_tree().process_frame
		bolt_contact = hand_rig.call("finger_contact_report", "R") as Dictionary
...
func _probe_engine(boat: RigidBody3D) -> void:
	...
	if room == null:
		print("motor odasi yok"); _quit_cleanly(); return
